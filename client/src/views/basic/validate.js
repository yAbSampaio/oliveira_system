...
export const validate_cpf = (cpf, message) => {
  ...
  if (!digitos_iguais) {
    ...
    if (resultado != digitos.charAt(0)) return message + " -> CPF Inválido";
    numeros = cpf.substring(0, 10);
    soma = 0;
    for (i = 11; i > 1; i--) soma += numeros.charAt(11 - i) * i;
    resultado = soma % 11 < 2 ? 0 : 11 - (soma % 11);
    if (resultado != digitos.charAt(1)) return message + " -> CPF Inválido";
    return message;
...
export const validate_address = (cep, street, house, district, message) => {
